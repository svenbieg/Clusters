...
public:
	...
	virtual _item_t* get(_key_t const& key, _item_t&& create, bool* created, bool again)=0;
...
public:
	...
	_item_t* get(_key_t const& key, _item_t&& create, bool* created, bool again)override
		{
		bool exists=false;
		uint16_t pos=get_item_pos(key, &exists);
		if(exists)
...
private:
	...
	// Modification
	_item_t* get_internal(_key_t const& key, _item_t&& create, bool* created, bool again)
		{
		uint16_t pos=0;
		uint16_t count=get_item_pos(key, &pos, false);
		if(!again)
...
public:
	...
	template <typename _item_param_t> bool set(_item_param_t&& item)
		{
		_item_t create(std::forward<_item_param_t>(item));
		bool created=false;
		get_internal(std::forward<_item_t>(create), &created);
